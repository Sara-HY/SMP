...
def predict_conv_net(testdata, U, img_w = 100, filter_hs = [3, 4, 5], hidden_units = [50, 2], dropout_rate = [0.5], batch_size = 100,
...
if __name__=="__main__":
    ...
    # y_test = np.array(list(np.ones(53454)) + list(np.zeros(173274)))  # the actual test classes
    # testdata = make_idx_data_cv(revs, word_idx_map, max_l=500, k=100, filter_h=5)
    # y_prediction = predict_conv_net(testdata, W, filter_hs=[3, 4, 5], conv_non_linear="relu", hidden_units=[50, 2],
    ...
    y_prediction = predict_conv_net(testdata, W, filter_hs=[3, 4, 5], conv_non_linear="relu", hidden_units=[50, 2],
                                     test_batch_size=500, batch_size=100, dropout_rate=[0.5], model_params=model_params)
    with open('./genderPredict', 'w') as f:
        for index in xrange(0, len(y_prediction)):
            # if y_prediction[index] == 1:
            #     f.write(revs[index]["uid"] + ',f' + '\n')
            # else:
            #     # print revs[index]["uid"] + ",m" + "\n"
            #     f.write(revs[index]["uid"] + ',m' + '\n')
