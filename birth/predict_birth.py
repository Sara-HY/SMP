...
def predict_conv_net(testdata, U, img_w = 100, filter_hs = [3, 4, 5], hidden_units = [50, 3], dropout_rate = [0.5], batch_size = 100,
    ...
    print 'predicting ...'
    # prediction = test_model_all(test_set_x)
...
if __name__=="__main__":
    ...
    # y_prediction = predict_conv_net(testdata, W, filter_hs=[3,4,5], conv_non_linear="relu", hidden_units=[50,3],batch_size=100,
    #                                 dropout_rate=[0.5], model_params = model_params)
    # acc = np.sum(y_test == y_prediction, axis=0) * 100 / float(len(y_test))
    # print 'Test Accuracy' + ' : ' + str(acc) + ' %'
    # print y_prediction
    testdata = make_idx_data_cv(revs, word_idx_map, max_l=500, k=100, filter_h=5)
    y_prediction = predict_conv_net(testdata, W, filter_hs=[3, 4, 5], conv_non_linear="relu", hidden_units=[50, 3],
                                     test_batch_size=500, batch_size=100, dropout_rate=[0.5], model_params=model_params)
    with open('./birthPredict', 'w') as f:
        for index in xrange(0, len(y_prediction)):
            # if y_prediction[index] == 2:
            #     f.write(revs[index]["uid"] + ',-1979' + '\n')
            # elif y_prediction[index] == 1:
            #     f.write(revs[index]["uid"] + ',1980-1989' + '\n')
            # else:  
            #     f.write(revs[index]["uid"] + ',1990+' + '\n')
